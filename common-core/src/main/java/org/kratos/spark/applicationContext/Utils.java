...
public class Utils {
	...
	static public void storeCustomersDataToHbase(Dataset<Row> dataset) throws IOException, Exception {

		Configuration conf = app.getHBaseNewAPIConfiguration();
		conf.set(TableOutputFormat.OUTPUT_TABLE, ConfigHandler.HBASE_CUSTOMERS_TABLE_NAME);

		JavaPairRDD<ImmutableBytesWritable, Put> hbasePuts = dataset.toJavaRDD().mapToPair(new PairFunction<Row, ImmutableBytesWritable, Put>() {
